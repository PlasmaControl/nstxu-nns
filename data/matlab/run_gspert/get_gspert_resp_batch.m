...
if 0
  % job_args.DATA_ROOT = getenv('DATA_ROOT');
  % job_args.MDSPLUS_DIR = getenv('MDSPLUS_DIR');
  % job_args.shot = 203172;
  % job_args.saveit = 0;
  % job_args.save_fn = 
end
...
targs.gspert_erred = false(size(times));
...
for i = 1:length(times)
  ...
  try
    ...
    f2copy = {'drdis', 'drdbetap', 'drdli', 'drdip', 'dzdis', 'dzdbetap', 'dzdli', ...
    ...
    for j = 1:length(f2copy)
      fn = f2copy{j};
      try
        targs.(fn)(i,:) = sys.gspert_data.(fn);
      catch
        targs.(fn)(i,:,:) = sys.gspert_data.(fn);
      end
    end
